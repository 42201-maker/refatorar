...
export async function createTaskRequest(taskData: TaskFormData): Promise<Task> {
  ...
  if (!response.ok) throw new Error(await extractErrorMessage(response, 'Erro ao criar tarefa'));
  return response.json();
}
...
export async function updateTaskRequest(id: number, taskData: TaskFormData): Promise<Task> {
  ...
  if (!response.ok) throw new Error(await extractErrorMessage(response, 'Erro ao atualizar tarefa'));
  return response.json();
}
